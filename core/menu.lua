...
--- myMenu:Item(UIMenuItem) â†’ ajoute un item deja crée
-- myMenu:Button("text", "desc") â†’ crÃ©e + ajoute un DrawButton
---@param text string
---@param description string
--function Menu:Button(text, description)
   -- return self:AddItem(UIMenuButton.New(text, description))
--end

--- Bouton simple ou avec action
-- Note: si enabled est false, le bouton est grisé et non sélectionnable. Si actions.onSelect est défini, il sera appelé lorsque le bouton est sélectionné, avec le menu et l'item en paramètre.
-- Exemple d'utilisation: myMenu:Button("Start Game", "Start a new game", true, { onSelect = function(menu, item) print("Starting game...") end })
-- Note: pour les autres types d'items (list, checkbox, slider, etc.), les actions peuvent inclure onChange (appelé lorsque la valeur change) et onSelect (appelé lorsque l'item est sélectionné). Les paramètres passés aux callbacks peuvent varier en fonction du type d'item et de l'action (ex: onChange pour une list peut recevoir le nouvel index de la liste, onChange pour un checkbox peut recevoir la nouvelle valeur cochée, etc.).
---@param text        string
---@param description string|nil
---@param enabled     boolean|nil
---@param actions     table|nil   { onSelect=fn }
function Menu:Button(text, description, enabled, actions)
    return self:AddItem(UIMenuButton.New(text, description, enabled, actions))
end
...
function Menu:List(text, items, index, description, actions)
    return self:AddItem(UIMenuList.New(text, items, index, description, actions))
end
...
---@param description string|nil
---@param actions     table|nil   { onChange=fn, onSelect=fn }
function Menu:Checkbox(text, checked, description, actions)
    return self:AddItem(UIMenuCheckbox.New(text, checked, description, actions))
end
...
function Menu:SliderProgress(text, progressStart, progressMax, description, style, enabled, actions)
    return self:AddItem(UIMenuSliderProgress.New(text, progressStart, progressMax, description, style, enabled, actions))
end
...
function Menu:Heritage(text, min, max, value, step, description, style, enabled, actions)
    return self:AddItem(UIMenuSliderHeritageItem.New(text, min, max, value, step, description, style, enabled, actions))
end
...
function Menu:GoUp()
    ...
    -- Skip les separators (jump)
    local attempts = 0
    while attempts < #self.items do
        local item = self.items[newIndex]
        if not item or not item.isSeparator then break end
        newIndex = newIndex - 1
...
function Menu:_Recalculate()
    ...
    -- Recalculer les positions des items
...
function Menu:_DrawHeader()
    ...
    local menuWidth = cfg.size.width

    local fallbackColor = cfg.background or (cfg.gradient and cfg.gradient.start) or { r = 0, g = 0, b = 0, a = 255 }

    -- Sprite (banniere) + tint optionnel
    if cfg.sprite and cfg.sprite.use then
       ...
       local ok = Draw.Sprite(
        ...
        if not ok then
       Draw.Rect(self.x, self.y, cfg.size.width, cfg.size.height, fallbackColor)
    end
    else
        Draw.Rect(self.x, self.y, cfg.size.width, cfg.size.height, fallbackColor)
    end
    ...
    -- Titre
    local titleIsCentered = (cfg.title and cfg.title.alignment == Text.Align.Center) or (cfg.title and cfg.title.alignment == 1)
    local titleX
    if titleIsCentered then
        titleX = self.x + (menuWidth * 0.5)
    else
        titleX = (cfg.title and cfg.title.offsetX ~= nil) and (self.x + cfg.title.offsetX) or self.x
    end

    local titleY = (cfg.title and cfg.title.offsetY ~= nil) and (self.y + cfg.title.offsetY) or self.y
    ...
    Text.Draw(
        ...
        cfg.title.color,
        cfg.title.alignment,
        cfg.title.shadow and cfg.title.shadow.enabled and cfg.title.shadow or nil
    )
end
...
function Menu:_DrawSubtitle()
    ...
    local yPos = self.y + Config.Header.size.height

    -- Largeur unique du menu: liÃ©e au header
    ...
    local subtitleX = self.x + ((cfg.text and cfg.text.offsetX ~= nil) and cfg.text.offsetX or 0)
    local subtitleY = yPos + ((cfg.text and cfg.text.offsetY ~= nil) and cfg.text.offsetY or 0)

    -- Texte subtitle (alignÃ© Ã  gauche)
    Text.Draw(
        self.subtitle,
        subtitleX,
        subtitleY,
        ...
        cfg.text.size,
        cfg.text.color,
        Text.Align.Left  -- âš¡ Explicitement Ã  gauche
    )
    
    -- Counter (1/7) alignÃ© Ã  droite
    if #self.items > 0 then
        local counterText = string.format("%d / %d", self.currentItem, #self.items)

        local paddingRight = (cfg.padding and cfg.padding.right) or 8
        local counterX = self.x + ((cfg.counter and cfg.counter.offsetX ~= nil) and cfg.counter.offsetX or (menuWidth - paddingRight))
        local counterY = yPos + ((cfg.counter and cfg.counter.offsetY ~= nil) and cfg.counter.offsetY or 0)

        Text.Draw(counterText,counterX,counterY,cfg.counter.font,cfg.counter.size,cfg.counter.color,Text.Align.Right) -- âš¡ Explicitement Ã  droite
...
function Menu:_DrawItemsBackground()
    ...
    -- Calculer la hauteur totale des items visibles
    local visibleItems = math.min(#self.items, self.maxItemsOnScreen)
    local itemHeight = (Config.Layout and Config.Layout.itemHeight) or 35
    local totalHeight = visibleItems * itemHeight
...
function Menu:_DrawItems()
    ...
    for i = startIndex, endIndex do
        ...
        local itemY = yStart + (i - startIndex) * itemHeight
        ...
        if isSelected and not item.isSeparator then
            self:_DrawNavigationHighlight(itemY, itemHeight, bgCfg)
        end
...
function Menu:_DrawDescription(description)
    ...
    local itemHeight = (Config.Layout and Config.Layout.itemHeight) or 35
    local yPos = self.y + Config.Header.size.height + Config.Subtitle.size.height +
        (visibleItems * itemHeight) + spacing
